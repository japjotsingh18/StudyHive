...
| Web production build | PENDING | Local sandbox blocks Turbopack's internal worker port; hosted CI required |
| Container image builds | PENDING | Hosted CI required |
| Dependabot alerts | PENDING | GitHub recalculates alerts after the branch is published |
| CodeQL and secret scanning | PENDING | Required protected-branch checks |
...
source or dependency diagnostic. The protected GitHub Actions build remains mandatory before release.

## Conclusion

Local compatibility checks pass, but v0.1.1 is not releasable until hosted CI, CodeQL, container
builds, and Dependabot alert recalculation complete successfully.
